...
session = web.session.Session(app, web.session.DiskStore('session'), initilizer={'usuario':''}
...
class index:
	def GET(self): 
		...
		if usuario:
			...
			return render.login(form = form, usuario = usuario) #and call te HTML file with params form and usuario

	def POST(self):
		form = login_form() #Catch the form
		if not form.validates(): #Check the validation options are correct
			return render.login(form = form, usuario = '') #Return the form again and erase the user data
